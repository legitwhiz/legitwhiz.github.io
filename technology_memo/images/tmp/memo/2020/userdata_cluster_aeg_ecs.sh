...
echo ECS_AVAILABLE_LOGGING_DRIVERS=[\""] >> /etc/ecs/ecs.config

## install packages
yum install -y openldap sssd-common sssd sssd-client sssd-ldap oddjob-mkhomedir expect aws-cli gcc mailx sysstat dstat
yum erase -y nscd
...
sed 's/#GSSAPIAuthentication no/GSSAPIAuthentication yes/g' | sed 's/#GSSAPICleanupCredentials yes/GSSAPICleanupCredentials no/g' > /etc/ssh/sshd_config
...
# install twistlock
##############################################
mkdir -p /tmp/work/twistlock
cd /tmp/work/twistlock
curl -k -u Defender-deployer:Ariake2blue! https://${twistlock_hostname}:8083/api/v1/scripts/defender.sh -o defender.sh
