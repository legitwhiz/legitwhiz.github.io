...
smitty alog              #Alogファイルの設定。
smitty dump              #システム・ダンプの特性の変更･表示。
```
...
smitty rmlv              #論理ボリュームの除去。
smitty lslv              #論理ボリュームの特性の表示。
...
smitty list_installed    #インストール済みソフトウェアおよび関連情報の表示。
smitty list_media        #メディア上のソフトウェアおよび関連情報の表示。
